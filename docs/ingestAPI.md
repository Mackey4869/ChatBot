...
- 実装: `frontend/utils/textSplit.ts`
- デフォルト: `chunkSize = 500` 文字、`overlap = 100` 文字
- 実際のスライド幅: `chunkSize - overlap = 400` 文字ずつ進む（デフォルト時）
- 特性: 現在は句読点（日本語: `。！？`、英語: `.!?`）を基準に文単位で分割し、文脈を維持するようにチャンクを作成します。句点は各チャンクに残す実装です。
- 長い単一文が `chunkSize` を超える場合は強制的に分割し、オーバーラップ分を繋げて次チャンクを開始します（稀なケースへのフォールバック）。
- この変更により、チャンク内の文脈が改善され、埋め込みと類似検索の精度向上が期待できます。必要なら `chunkSize` / `overlap` を調整してください。
